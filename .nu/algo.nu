...
def root [] {
  $env.ALGO_ROOT? | default (
    if ($env.PWD | path join ".nu/algo.nu" | path exists) {
      $env.PWD
    } else {
...
# Call the TypeScript cognitive engine CLI. Returns parsed JSON or raw string.
def engine [subcommand: string, ...args: any] {
  let root_path = (root)
  let result = (
    ^bun run $"($root_path)/packages/engine/src/cli.ts" $subcommand ...$args
    | complete
  )
  if ($result.exit_code != 0) {
    print $"(engine stderr: ($result.stderr))"
    return null
  }
...
export def "algo add" [] {
  ...
  let topics = (ask "主題標籤 (逗號分隔)") | split row "," | each { str trim | str lowercase } | where ($it | is-not-empty)
  ...
  let err = (ask "主因分類 [R/K/P/M/I/B/E/T] (乾淨AC可空)") | str uppercase
  let err2 = (ask "次因 (可空)") | str uppercase
...
# 今日到期的複習（空白重推）
export def "algo due" [] {
  # FSRS-based due reviews from engine
  let dueReviews = (engine get-due-reviews)
  if ($dueReviews == null) { return [] }
  $dueReviews
  | each {|r|
      {
        conceptId: $r.conceptId
        retrievability: ($r.retrievability | math round --precision 2)
...
# 記錄一次複習結果
export def "algo done" [id: string, --failed] {
...
# 近況統計
export def "algo stats" [--days (-d): int = 30] {
...
def load-session [] {
  ...
  if not ($sf | path exists) {
    error make { msg: "沒有進行中的 session，先 algo start" }
  }
  open $sf
}
...
export def "algo start" [
  ...
  let sf = (session-file)
  if ($sf | path exists) {
    error make { msg: $"已有進行中 session：(open $sf | get problem)。先 algo finish 或 algo abort" }
  }
  ...
  let active_file = if (root | path join "work/sol.cpp" | path exists) { "work/sol.cpp" } else { null }
  {
    problem: $problem
    rating: $rating
    topics: ($topics | split row "," | each { str trim | str lowercase } | where ($it | is-not-empty))
    mode: $mode
  ...
  # Check FSRS due reviews
  let due = (engine get-due-reviews)
  if ($due != null and ($due | length) > 0) {
    print $"⚠ 有 ($due | length) 個概念到期複習（retrievability < 0.7）"
    for r in ($due | first 3) {
      print $"   - ($r.conceptId): R=($r.retrievability | math round --precision 2)"
    }
  }
...
規則：用繁體中文，簡潔。嚴禁輸出高於本級的任何資訊。不得評論學生應該早點看題解。"
  ...
  let cmd = ($env.ALGO_LLM_CMD? | default "")
  if ($cmd | is-empty) {
    print "--- 未設定 ALGO_LLM_CMD，把以下提示詞貼給你的 LLM ---"
    print $prompt
  } else {
    $prompt | ^sh -c $cmd
  }
...
export def "algo finish" [
  ...
  let err_v = (if $err == null { ask "主因分類 [R/K/P/M/I/B/E/T] (乾淨AC可空)" } else { $err }) | str uppercase
  let err2_v = (if $err2 == null { ask "次因 (可空)" } else { $err2 }) | str uppercase
  ...
  # ── Engine integration: update BKT + phase transition ──────
  if ($s.topics | is-not-empty) {
    let engResult = (engine finish-session $s.problem $final_result ($s.topics | to json) $t_think $t_code $t_debug)
    if ($engResult != null and $engResult.success? == true) {
      for u in $engResult.updates {
        if ($u.from != $u.to) {
          let pLVal = ($u | get pL)
          let pLStr = ($pLVal | math round --precision 2 | into string)
          let cid = ($u | get conceptId)
          let fromP = ($u | get from)
          let toP = ($u | get to)
          print ("  ↳ " + $cid + ": " + $fromP + " → " + $toP + " (P(L)=" + $pLStr + ")")
        }
      }
    }
  }
...
# 放棄目前 session（不寫日誌）
export def "algo abort" [] {
  let s = (load-session)
  rm (session-file)
  print $"已放棄 ($s.problem)（未寫入日誌）"
}

# 重置學習進度（BKT + FSRS + phase）
export def "algo reset" [
  scope: string = "all"  # "all" 或特定概念 ID（如 "recursion"）
] {
  let result = (engine reset-progress $scope)
  if ($result != null and $result.success? == true) {
    if ($scope == "all") {
      print "已重置所有學習進度（BKT / FSRS / phase）"
    } else {
      print $"已重置 ($scope) 的學習進度"
    }
  } else {
    print "重置失敗"
  }
...
export def "algo profile" [--rating (-r): int] {
...
export def "algo pick" [
  --count (-c): int = 3
  --rating (-r): int     # 不給則讀 algo profile
...
# 週報 + 教練診斷 prompt → ALGO_LLM_CMD（未設定則印出）
export def "algo diagnose" [--days (-d): int = 7] {
  let md = (algo report --days $days)
...
輸出不超過 400 字。"
  let cmd = ($env.ALGO_LLM_CMD? | default "")
  if ($cmd | is-empty) {
    print "--- 未設定 ALGO_LLM_CMD，把以下貼給你的 LLM ---"
    $prompt
  } else {
    $prompt | ^sh -c $cmd
  }
...
- 只輸出程式碼本身，不要 markdown 圍欄、不要解釋"
  let cmd = ($env.ALGO_LLM_CMD? | default "")
  if ($cmd | is-empty) {
    print "--- 未設定 ALGO_LLM_CMD，把以下貼給你的 LLM，回覆存成 gen.cpp ---"
    return $prompt
  }
...
export def "algo stress" [
  ...
  print "編譯中..."
  ^g++ -O2 -std=c++17 -o /tmp/algo_sol $sol
  ^g++ -O2 -std=c++17 -o /tmp/algo_brute $brute
  ^g++ -O2 -std=c++17 -o /tmp/algo_gen $gen

  for i in 1..$runs {
    let inp = (^/tmp/algo_gen $i)
    let o1 = ($inp | ^/tmp/algo_sol | str trim)
    let o2 = ($inp | ^/tmp/algo_brute | str trim)
    if $o1 != $o2 {
...
def unit-remaining [u: record] {
  let done = (solved-ids)
  let practice = ($u.phase_practice?.problems? | default [])
  let exam = ($u.phase_exam?.problems? | default [...$practice])
  [...$practice, ...$exam] | where $it not-in $done | uniq
}

# 當前單元的 LLM 家教課（會的快速複習、不會的從零教）
export def "algo learn" [
  --unit (-u): int
  --no-source     # 不抓 USACO Guide 教材（離線/省流量）
] {
  let prof = (algo profile)
  ...
  let learn_extra = if ($ud.phase_learn? | default {} | get -o topics | length) > 0 {
    ...
    [$"\n\n## 本課語法點與跟打練習" "教學順序：逐一教以下每個語法點。先展示語法模板（在對話中），要求學生在 vim 中自己打一遍（可以用 work/templates/ 下的檔案或自己開新檔）。學生說「看」你就讀代碼，然後 algo run 給他看輸出。確認學生理解每個語法點（依 evidence 檢核）後才進入下一個。" "所有語法點教完後，才進入檢核題階段。" "" $raw $ref] | where ($it | is-not-empty) | str join "\n\n"
  } else { "" }
...
- **學生問『某某語法是什麼/怎麼寫』時，不得直接回答。** 用系統工具（algo concept index / algo concept show / algo drill）找出對應概念 → 讓學生動手練。回答語法問題永遠等於害他——他需要的是肌肉記憶不是聽解釋。
- 學生在另一個 vim 窗口手打代碼（肌肉記憶必要）；學生說『看』你就讀代碼、要跑就 algo run，不叫學生開終端打指令
- 示例代碼用最小片段，要求學生自己動手打一遍；不要餵完整大段程式($learn_extra)($phase_note)
- 收尾時指派檢核題（進練習階段），開題交給系統，不劇透解法
- 全程繁體中文；一次訊息只推進一小步，等學生回應($source)"
  let cmd = ($env.ALGO_LLM_CMD? | default "")
  if ($cmd | is-empty) {
    print "--- 未設定 ALGO_LLM_CMD，把以下貼給你的 LLM 開始上課 ---"
    $prompt
  } else {
    $prompt | ^sh -c $cmd
  }
...
export def "algo anki" [--tsv: string] {
...
# 人類可讀的近期日誌
export def "algo log" [--limit (-l): int = 20] {
...
# algo concept fuzzy "前綴和邊界" -n "l..r 轉 pre[r]-pre[l-1] 還會猶豫"
# algo concept ok "前綴和邊界"
# algo concept list
export def "algo concept" [
...
# 列出概念索引（支援搜尋）
# algo concept index         → 所有概念
# algo concept index -q for  → 搜尋名稱含 "for" 的概念
export def "algo concept index" [
  --query (-q): string = ""
] {
...
# 顯示某個概念的完整 drill 資訊
# LLM 用這個取代「直接回答語法問題」
# algo concept show mt_u1_eof  → 顯示 syntax_template + drills
export def "algo concept show" [
  concept: string
] {
  let idx = (concept-index)
  ...
  if ($c | is-empty) {
    ...
    if ($fuzzy | is-empty) { error make { msg: $"找不到概念 '($concept)'。用 algo concept index 列出所有" } }
  ...
  let lines = ($c.drills | enumerate | each {|d|
    ...
    $"  ($d.index). [($tag)] ($d.item.desc) → algo drill ($c.id) --mode ($d.item.type)" + (if $d.item.type == "problem" { $" --problem ($d.item.id)" } else { "" })
  } | str join "\n")
  print $lines
}
...
export def "algo drill" [
  ...
  let c = ($idx | where id == $concept)
  if ($c | is-empty) {
    error make { msg: $"找不到概念 '($concept)'。用 algo concept index 列出所有" }
  }
  let c = ($c | first)
  ...
  match $mode {
    ...
    "problem" => {
      ...
      print $"algo start ($pid) --phase practice"
      print (algo start $pid --phase practice)
    }
  }
}
...
export def "algo run" [
  ...
  if not ($f | path exists) { error make { msg: $"找不到 ($f)" } }
  let comp = (do { ^g++ -O2 -std=c++17 -Wall -Wextra -o /tmp/algo_run $f } | complete)
  if $comp.exit_code != 0 {
    return { compiled: false, errors: $comp.stderr }
  }
  let t0 = (date now)
  let r = (do { $input | ^timeout 5 /tmp/algo_run } | complete)
  {
    compiled: true
...
# algo bench work/sol.cpp -i "3"    → 單筆含詳細資源
# algo bench work/sol.cpp -t 5      → 連續跑 5 次取統計
export def "algo bench" [
  file: string = "work/sol.cpp"
  --input (-i): string = ""       # 單筆測資
  ...
  let comp = (do { ^g++ -O2 -std=c++17 -Wall -Wextra -o /tmp/algo_bench $f } | complete)
  if $comp.exit_code != 0 {
    return { compiled: false, errors: $comp.stderr }
  }
  ...
  let timing = (1..$times | each {|i|
    ...
    let r = (do { $inp | ^timeout 5 /tmp/algo_bench } | complete)
...
export def "algo today" [] {
  ...
  if $cur <= ($c | length) {
    ...
    print "■ 課綱已完成，今日題單："
    try { print (algo pick | table) } catch { print "  （先 just sync，再 just profile -r <rating>）" }
  }
...
export def "algo fmt" [
  ...
  let f = if ($file | path exists) { $file } else { root | path join $file }
  if not ($f | path exists) { error make { msg: $"找不到 ($f)" } }
  let laid = (mktemp -t algo_fmt.XXXX.cpp)
...
export def "algo doctor" [] {
  ...
  # 功能：編譯器真的能編
  "int main(){return 0;}" | save -f /tmp/algo_doctor.cpp
  let comp = (do { ^g++ -o /tmp/algo_doctor /tmp/algo_doctor.cpp } | complete)
  $rows = ($rows | append { check: "功能 g++ 編譯", ok: ($comp.exit_code == 0), required: true, fix: "檢查 gcc 安裝" })

  # 功能：提示閘道邏輯（隔離 ALGO_ROOT，不污染日誌）
  let algo_mod = (root | path join ".nu/algo.nu")
  let tmp = (mktemp -d)
  mkdir ($tmp | path join "log")
  let gate = (with-env { ALGO_ROOT: $tmp } {
    do { ^nu -c $"use '($algo_mod)' *; algo start doctor/test --stuck-min 30; algo hint" } | complete
  })
  rm -rf $tmp
  $rows = ($rows | append { check: "功能 提示閘道", ok: ($gate.stdout | str contains "閘道拒絕"), required: true, fix: "algo.nu 損壞，git checkout" })
